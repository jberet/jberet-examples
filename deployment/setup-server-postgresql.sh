...
# Switch to use the below command if you are using the `postgresql-datasource` layer instead of the `postgresql-driver` layer:
#$JBOSS_HOME/bin/jboss-cli.sh -c --commands='/subsystem=batch-jberet/jdbc-job-repository=batch_db:add(data-source=PostgreSQLDS), /subsystem=batch-jberet/:write-attribute(name=default-job-repository,value=batch_db),reload'